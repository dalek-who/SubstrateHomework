...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn transfer_from(_origin, from: T::AccountId, to: T::AccountId, value: T::TokenBalance) -> DispatchResult {
            ...
            <Allowance<T>>:: insert((from.clone(), to.clone()), update_allowance);
        ...
        fn approve(_origin, spender: T::AccountId, value: T::TokenBalance) -> DispatchResult {
            ...
            let update_allowance = allowance + value;
            <Allowance<T>>::insert((sender.clone(), spender.clone()), update_allowance);
...
impl<T: Trait> Module<T> {
    fn _transfer(
        ...
        let update_from_balance = sender_balance.checked_sub(&value).ok_or(Error::<T>::StorageOverflow)?;
        let receiver_balance = Self::balance_of(to.clone());
        let update_to_balance = receiver_balance.checked_add(&value).ok_or(Error::<T>::StorageOverflow)?;

        <BalanceOf<T>>::insert(from.clone(), update_from_balance);
